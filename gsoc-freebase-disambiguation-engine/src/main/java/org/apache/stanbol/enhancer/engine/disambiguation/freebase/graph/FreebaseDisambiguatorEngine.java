...
@Service
@Properties(value = { 
        @Property(name = EnhancementEngine.PROPERTY_NAME, value = "freebase-disambiguation") ,
        @Property(name = FreebaseDisambiguatorEngine.GRAPH_LOCATION)
})

public class FreebaseDisambiguatorEngine extends AbstractEnhancementEngine<IOException, RuntimeException> implements
        EnhancementEngine, ServiceProperties
{
    ...
    /**
     * <p>Graph (Neo4jGraph) containing the whole graph</p>
     */
    private Graph graph;
    
    /*
     * The following parameters describe the ratio of the original fise:confidence values and the disambiguation scores
     * contributing to the final disambiguated fise:confidence
     * 
     ...
     * </p>
     * 
     * @param ci The ContentItem to be enhanced
     * 
     * @return An integer indicating whether the ContentItem can be enhanced or not. There are a few constants that can
     *         be returned: CANNOT_ENHANCE, ENHANCE_SYNCHRONOUS, ENHANCE_ASYNC
     */
    ...
    /**
     * <p>
     * This function first evaluates all the possible ambiguations of each text annotation detected. The ids of the
     * referenced entities are used to check relations in the Freebase graph in order to obtain a new disambiguation
     * score based on the relations between entities in the graph
     * </p>
     * 
     * <p>
    ...
    public void computeEnhancements(ContentItem ci) throws EngineException
    {
         ...
         * 
         * 2. Generate all the possible solutions for the text. That means,
         *    generate the combination of entities of each set (cartesian product)
         * 
         * 3. Construct the subgraph from the whole graph using only the extracted entities
         * 
         * 4. Filter the possible solutions, removing those entities from each possible solution which are not valid, i.e isolated entities in the graph
         *    
         * 5. For each possible solution, calculate the shortest path between every pair of entities in it and store the disambiguation score for the possible solution
         * 
         * 6. For each entity annotation, modify the confidence value using the disambiguation score for the suggestion of the entity annotation
         * 
         */
        ...
        /*
         * (1)
         * Generating the Map TextAnnotation->List<UriRef EntityUri> to be used to generate the possible solutions
         * using cartesian product (one element for each entity of every text annotation)
         * 
         ...
         */
        Set<List<Suggestion>> possibleSolutions = FreebaseDisambiguatorEngineHelper.generatePossibleSolutions(taSuggestions);
         ...
         */
        possibleSolutions = FreebaseDisambiguatorEngineHelper.filterPossibleSolutions(possibleSolutions, subgraph);
        
        /*
         * (5) Calculate the shortest-path between entities in the possible solution and modify the disambiguation score of each Suggestion
         */
    ...
    /**
     * <p>Calculates the new confidence value for each Suggestion using the normalized disambiguation score of the Suggestion
     * <p>Applies the disambiguation results to the enhancements, modifying the confidence value of the entity annotations</p>
     * 
     * @param metadata the {@code MGraph} instance containing the enhancement triples
     * @param disData the {@code DisambiguationData} instance containing the Suggestion objects and their normalized disambiguation scores
     */
    private void disambiguateAndApplyResults(MGraph metadata, DisambiguationData disData)
    {
        for (SavedEntity savedEntity : disData.textAnnotations.values()) {
            for (Suggestion suggestion : savedEntity.getSuggestions()) {
                ...
                /*
                 * Suggestions with null in disambiguated confidence means that they don't form part of any possible solution
                 * so their confidence value won't change
                 */
                if(ns == null) {
                ...
                if (suggestion.getDisambiguatedConfidence() != null) {
                    ...
                    log.info("Modifying confidence for "+suggestion.getEntityAnnotation().getUnicodeString()+" (Entity "+suggestion.getEntityUri().getUnicodeString()+") -> Last confidence: "+suggestion.getOriginalConfidnece()+" - New confidence: "+suggestion.getDisambiguatedConfidence());
                    EnhancementEngineHelper.set(metadata, suggestion.getEntityAnnotation(), ENHANCER_CONFIDENCE,
                        suggestion.getDisambiguatedConfidence(), literalFactory);
                    EnhancementEngineHelper.addContributingEngine(metadata, suggestion.getEntityAnnotation(), this);
                }
            }
        }
    ...
    /**
     * <p>Dump the graph to the log debug</p>
     * 
     * @param graph the {@code Graph> to dump
     */
    private void dumpGraph(Graph graph) {
        if(log.isDebugEnabled() || true) {
            ...
            {
                log.debug("Vertex " + vertex + " : " + vertex.getProperty(FreebaseDisambiguatorEngineConstants.VERTEX_ENTITY_URI_PROPERTY));
            }
    ...
    protected void activate(ComponentContext ce) throws ConfigurationException
    {
        try
        {
            ...
            // }
            if(properties.get(GRAPH_LOCATION) != null && !((String) properties.get(GRAPH_LOCATION)).isEmpty()) {
                ...
                throw new IOException(GRAPH_LOCATION+" property is null or empty. Failed to initialize the engine");
        }
        catch (IOException e)
        { // log
